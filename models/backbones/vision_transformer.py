...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.outplanes = [embed_dim]*len(outlayers)
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.blocks):
            x = layer(x)
            if self.cls_token is not None:
                outs.append(rearrange(x[:,1:,:], "b (h w) c -> b c h w", h=self.feature_size[0]))
            else:
                outs.append(rearrange(x, "b (h w) c -> b c h w", h=self.feature_size[0]))
        # x = self.norm(x)
        # return x
        return outs
    ...
    def forward(self, inputs):
        image = inputs["image"]
        outs = self.forward_features(image)
        features = [outs[i-1] for i in self.outlayers]
...
def _create_vision_transformer(variant, pretrained=False, pretrained_model=None, **kwargs):
    ...
    if pretrained_model:
        pretrained_cfg["file"]=pretrained_model
